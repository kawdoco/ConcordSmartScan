...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .logo-icon {
    width: 36px; height: 36px; background: #1e3a8a;
  ...
  .avatar-sm {
    ...
    font-weight: 600; color: #1e40af; flex-shrink: 0;
  }
  ...
  .tab-btn:hover:not(.active) { background: #f1f5ff; border-color: #2563eb; color: #1e3a8a; }
`;
...
// ── Shared shell ──
function Shell({ children, searchPlaceholder = "Search by Garment ID, Style, or Material" }) {
  return (
    <>
      <style>{styles}</style>
        ...
        <div className="main">
          <header className="topbar">
            <div className="search-box">
              <Icon d={IC.search} size={14} />{searchPlaceholder}
            </div>
    ...
    <Shell>
      <div className="page-content">
        <div className="breadcrumb">
          <a href="#">Garments</a> › <span>Add New Garment</span>
        </div>
        <h1 className="page-title">Add New Garment</h1>

        <div className="form-card">
          {/* Garment Details */}
          <div className="section-heading">
            <Icon d={IC.garments} size={16} />Garment Details
          </div>

          <div className="form-row full">
            <div className="form-group">
              <label>Garment Name</label>
              <input type="text" placeholder="Concord – Colombo"
          ...
          <div className="form-row">
            <div className="form-group">
              ...
              <input type="text" value="GR012" disabled />
              <span className="field-hint">Auto-generated system ID</span>
            </div>
            <div className="form-group">
              <label>Phone Number</label>
              <input type="tel" placeholder="e.g. +94 11 234 5678"
          ...
          <div className="form-row full">
            <div className="form-group">
              <label>Address</label>
              <textarea placeholder="Enter full manufacturing unit address..."
          ...
          <div className="section-heading">
            <Icon d={IC.location} size={16} />Location Coordinates
          </div>

          <div className="form-row">
            <div className="form-group">
              <label>Latitude</label>
              <input type="text" placeholder="e.g. 6.9271"
                value={form.lat} onChange={set("lat")} />
            </div>
            <div className="form-group">
              <label>Longitude</label>
              <input type="text" placeholder="e.g. 79.8612"
          ...
          <div className="form-actions">
            ...
            <button className="btn-primary">
              <Icon d={IC.plus} size={14} />Add Garment
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="page-content">
        ...
        <div className="form-card">
          ...
          <div className="form-actions">
            <button className="btn-cancel">Cancel</button>
            <button className="btn-primary">
              <Icon d={IC.edit} size={14} />Update Garment
            </button>
          </div>
        </div>
      </div>
...
// ── Demo wrapper: toggle between both pages ──
export default function GarmentPages() {
  const [tab, setTab] = useState("add");
  return (
    <div>
      <div style={{ padding: "16px 32px", background: "#1e3a8a", display: "flex", gap: 12 }}>
        <button className={`tab-btn ${tab === "add" ? "active" : ""}`}
          onClick={() => setTab("add")} style={{ fontFamily: "'DM Sans', sans-serif" }}>
          + Add Garment
        </button>
        <button className={`tab-btn ${tab === "edit" ? "active" : ""}`}
          onClick={() => setTab("edit")} style={{ fontFamily: "'DM Sans', sans-serif" }}>
          ✏ Edit Garment
        </button>
      </div>
      {tab === "add" ? <AddGarmentPage /> : <EditGarmentPage />}
    </div>
  );
}
